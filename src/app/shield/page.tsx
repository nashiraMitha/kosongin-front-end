...
export default function ImpulseShieldPage() {
  ...
  // State Utama Data Shield (Di-lock pakai array kosong agar tidak undefined)
  ...
  // 1. Ambil Data Real-Time Sinkron dari Server API
  const refreshShieldData = async () => {
    try {
      ...
      if (Array.isArray(rawData)) {
        ...
        // Preserve optimistic temp items (id startsWith 'temp-') to avoid UI flicker when refreshing
        const preservedTemps = Array.isArray(shieldList) ? shieldList.filter((i: any) => typeof i?.id === 'string' && i.id.startsWith('temp-')) : [];
        const merged = [
          ...preservedTemps,
          ...normalized.filter((s: any) => !preservedTemps.some((t: any) => t.itemName === s.itemName && t.price === s.price))
        ];

        setShieldList(merged);
  ...
  const handleAddToWaitingList = async (e: React.FormEvent) => {
    ...
    // Optimistic Update: Masukkan ke UI detik ini juga biar gak kedip ilang
  ...
  return (
    <div className="min-h-screen bg-[#FEFEFE] flex flex-col font-sans pb-20 relative">
      ...
      {/* COMPONENT TOAST NOTIFIKASI HIJAU */}
      {toast.show && (
      ...
      <main className="px-6 md:px-12 lg:px-20 mt-10 space-y-10">
        ...
        {/* LAYOUT GRID UTAMA - MEMBERSIHKAN LAYOUT STRETCH GLITCH */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          ...
          <div className="space-y-8 h-auto">
            ...
            {/* BOX WAITING LIST KIRI BAWAH - REAL-TIME SINKRON */}
